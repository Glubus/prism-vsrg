...
impl SongList {
    ...
    pub fn render(
        ...
        let filtered_indices = &menu_state.filtered_indices;

        static mut LAST_LOG_FRAME: u32 = 0;
        unsafe {
            LAST_LOG_FRAME = (LAST_LOG_FRAME + 1) % 600;
            if LAST_LOG_FRAME == 0 {
                log::info!(
                    "SONGLIST: Rendering with filtered_indices len: {}, total beatmapsets: {}",
                    filtered_indices.len(),
                    menu_state.beatmapsets.len()
                );
            }
        }

        // Use filtered set for display
        ...
        ScrollArea::vertical()
            ...
            .show_viewport(ui, |ui, rect| {
                ...
                // We map VISUAL rows to ACTUAL indices
                let mut visual_rows: Vec<(usize, f32)> = Vec::new(); // (real_index, y_pos)

                for i in filtered_indices {
                    let (_, beatmaps) = &beatmapsets[*i];
                    visual_rows.push((*i, current_height));
                ...
                let min_visual_row = cumulative_heights
                    .iter()
                    .position(|&h| h >= rect.min.y)
                    .unwrap_or(0);
                let max_visual_row = cumulative_heights
                    .iter()
                    .position(|&h| h > rect.max.y)
                    .unwrap_or(cumulative_heights.len());

                let fill_top = cumulative_heights
                    .get(min_visual_row)
                    .copied()
                    .unwrap_or(0.0);
                ...
                let start_idx = min_visual_row.min(filtered_indices.len());
                let end_idx = max_visual_row.min(filtered_indices.len());

                for visual_idx in start_idx..end_idx {
                    let real_index = filtered_indices[visual_idx];

                    if let Some((beatmapset, beatmaps)) = beatmapsets.get(real_index) {
                        let id = real_index;
                ...
                self.min = min_visual_row;
                self.max = max_visual_row;
            });
