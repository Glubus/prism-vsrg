...
pub struct RenderResources {
    ...
    pub progress_buffer: wgpu::Buffer,

    // Legacy bind groups (will be replaced by SkinAssets)
...
impl RenderResources {
    ...
    fn reload_legacy_bind_groups(
        ...
        let receptor_color = self.skin.gameplay.receptors.color;
        let def_col = [
        ...
        for col in 0..key_count {
            ...
            let tex_n = loaded_tex.unwrap_or_else(|| {
                ...
                let g = note_color[1];
                let b = note_color[2];
                // Check brightness to avoid invisible black notes
                let brightness = (r + g + b) / 3.0;

                let color = if brightness < 0.1 {
                    log::warn!(
                        "RESOURCES: Note Col {} -> Color too dark/black ({:?}). Forcing WHITE for visibility.",
                        col,
                        note_color
                    );
                    [255, 255, 255, 255]
                ...
                create_default_texture(device, queue, color, "Def Note")
            });
        ...
        // Helper to create bind group from path
        ...
        // Reload special note types
        self.mine_bind_group =
            create_bind_group_from_path(self.skin.get_mine_image(key_count, 0), "Mine BG");
        self.hold_body_bind_group = create_bind_group_from_path(
            self.skin.get_hold_body_image(key_count, 0),
            "Hold Body BG",
        );
        self.hold_end_bind_group =
            create_bind_group_from_path(self.skin.get_hold_end_image(key_count, 0), "Hold End BG");
        self.burst_body_bind_group = create_bind_group_from_path(
            self.skin.get_burst_body_image(key_count, 0),
            "Burst Body BG",
        );
        self.burst_end_bind_group = create_bind_group_from_path(
            self.skin.get_burst_end_image(key_count, 0),
            "Burst End BG",
        );

        // Update bind groups
        self.note_bind_groups = note_bind_groups;
        self.receptor_bind_groups = receptor_bind_groups;
        self.receptor_pressed_bind_groups = receptor_pressed_bind_groups;

        // Update PlayfieldDisplay for the legacy system
        self.gameplay_view.set_column_count(key_count);
    }
    ...
    pub fn new(ctx: &RenderContext, egui_ctx: &egui::Context) -> Self {
        ...
        let receptor_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            ...
            size: (18 * std::mem::size_of::<InstanceRaw>()) as u64, // Max 18K support
            usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST,
            mapped_at_creation: false,
        });
        ...
        // Load all key modes at startup (4K to 18K cached)
        res.skin_assets = Some(SkinAssets::load_all(
            device,
            &ctx.queue,
            &mut res.skin,
    ...
    pub fn update_component_positions(&mut self, screen_width: f32, screen_height: f32) {
        ...
        use skin::hud::time_left::TimeDisplayMode as ConfigMode;
