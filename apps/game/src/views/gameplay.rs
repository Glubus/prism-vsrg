...
impl GameplayView {
    ...
    /// Updates the column count when switching key modes.
    pub fn set_column_count(&mut self, key_count: usize) {
        // Resize column_instances_cache if needed
        self.column_instances_cache
            .resize_with(key_count, || Vec::with_capacity(100));
        // Truncate if new key_count is smaller
        self.column_instances_cache.truncate(key_count);
        // Update playfield component
        self.playfield_component.set_column_count(key_count);
    }
