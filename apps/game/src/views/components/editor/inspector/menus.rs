...
pub fn edit_background(ui: &mut Ui, skin: &mut Skin) -> bool {
    let mut changed = false;

    section_header(ui, "🖼️ Background Image");

    // Convert PathBuf to String for image_picker
    let mut bg_string = skin
        .background
        .as_ref()
        .map(|p| p.to_string_lossy().to_string());

    if image_picker(ui, "Background", &mut bg_string, Some(&skin.base_path)) {
        // Convert back to PathBuf
        skin.background = bg_string.map(std::path::PathBuf::from);
        changed = true;
    }
