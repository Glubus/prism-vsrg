...
impl GlobalState {
    ...
    fn sync_db_to_menu(&mut self) {
        ...
        if let Ok(guard) = db_state_arc.try_lock() {
            ...
            if let AppState::Menu(menu) = &mut self.current_state {
                menu.db_status = guard.status.clone();
            }

            // Sync data only when Idle and version changed
    ...
    /// Sets the current key count and reloads keybinds for that mode.
    pub fn set_key_count(&mut self, key_count: usize) {
        if self.current_key_count != key_count {
            self.current_key_count = key_count;
            log::info!("LOGIC: Switching to {}K mode", key_count);
            if let Err(e) = self.input_cmd_tx.send(InputCommand::ReloadKeybinds(
                self.settings.keybinds.clone(),
                key_count,
            )) {
                log::error!("LOGIC: Failed to send keybinds for {}K: {}", key_count, e);
            }
        }
    }
    ...
    pub fn handle_action(&mut self, action: GameAction) {
        log::trace!("GLOBAL: Received Action: {:?}", action);
