...
impl HitWindow {
    ...
    pub fn find_best_note<N: NoteAccessor>(
        ...
        for i in start_index..notes.len() {
            ...
            // Early exit: if note is past search window
            if note.time_us() > search_limit {
                break;
            }

            // Skip if wrong column or already hit
            if note.column() != input_column || note.is_hit() {
                continue;
            }
...
/// Trait for accessing note data needed by hit matching.
/// Allows both NoteData and simple note representations to be used.
