...
impl Renderer {
    pub async fn new(window: Arc<Window>, menu_state: Arc<Mutex<MenuState>>) -> Self {
        ...
        let mut skin = Skin::load(&settings.current_skin).unwrap_or_else(|e| {
            eprintln!(
                "Error loading skin '{}': {}. Falling back to default.",
                settings.current_skin, e
            );
            Skin::load("default").unwrap_or_else(|e_def| {
                panic!(
                    "CRITICAL: Default skin is missing or corrupted! Error: {}",
                    e_def
                );
            })
        });
        ...
        let song_button_selected_texture =
            load_egui_texture(skin.song_button_selected.clone(), "song_button_selected");
        let difficulty_button_texture =
            load_egui_texture(skin.difficulty_button.clone(), "difficulty_button");
        let difficulty_button_selected_texture = load_egui_texture(
            skin.difficulty_button_selected.clone(),
        ...
        for col in 0..num_columns {
            let receptor_texture = if let Some(path) = skin.get_receptor_image(num_columns, col) {
                ...
                create_default_texture(&device, &queue, receptor_default_color, "Def Receptor")
            };
            let receptor_view =
            ...
            receptor_bind_groups.push(device.create_bind_group(&wgpu::BindGroupDescriptor {
                label: Some(&format!("Receptor BG {}", col)),
                layout: &bind_group_layout,
                entries: &[
                    wgpu::BindGroupEntry {
                        binding: 0,
                        resource: wgpu::BindingResource::TextureView(&receptor_view),
                    },
                    wgpu::BindGroupEntry {
                        binding: 1,
            ...
            let pressed_texture_path = skin
                .get_receptor_pressed_image(num_columns, col)
                .or_else(|| skin.get_receptor_image(num_columns, col));
            let pressed_texture = if let Some(path) = pressed_texture_path {
                load_texture_from_path(&device, &queue, &path)
                    .map(|(t, _, _)| t)
                    .unwrap_or_else(|| {
                        create_default_texture(
                ...
                create_default_texture(&device, &queue, receptor_default_color, "Def Pressed")
            };
            ...
            receptor_pressed_bind_groups.push(device.create_bind_group(
                &wgpu::BindGroupDescriptor {
                    label: Some(&format!("Receptor Pressed BG {}", col)),
                    layout: &bind_group_layout,
                    entries: &[
                        wgpu::BindGroupEntry {
                            binding: 0,
                            resource: wgpu::BindingResource::TextureView(&pressed_view),
                        },
                        wgpu::BindGroupEntry {
                            binding: 1,
                            resource: wgpu::BindingResource::Sampler(&sampler),
        ...
        for col in 0..num_columns {
            let note_texture = if let Some(path) = skin.get_note_image(num_columns, col) {
                load_texture_from_path(&device, &queue, &path)
                    .map(|(t, _, _)| t)
                    .unwrap_or_else(|| {
                        create_default_texture(&device, &queue, note_default_color, "Def Note")
                    })
            } else {
                create_default_texture(&device, &queue, note_default_color, "Def Note")
            };
            ...
            note_bind_groups.push(device.create_bind_group(&wgpu::BindGroupDescriptor {
                label: Some(&format!("Note BG {}", col)),
                layout: &bind_group_layout,
                entries: &[
                    wgpu::BindGroupEntry {
                        binding: 0,
                        resource: wgpu::BindingResource::TextureView(&note_view),
                    },
                    wgpu::BindGroupEntry {
                        binding: 1,
                        resource: wgpu::BindingResource::Sampler(&sampler),
        ...
        let mut hit_bar = HitBarDisplay::new(0.0, 0.0, 100.0, skin.config.hit_bar_height_px);
        ...
        engine.scroll_speed_ms = settings.scroll_speed;
