...
use crate::views::components::{
    PlayfieldDisplay, ScoreDisplay, AccuracyDisplay, 
    JudgementPanel, ComboDisplay, JudgementFlash, HitBarDisplay
};
...
impl RenderResources {
    pub fn new(ctx: &RenderContext, egui_ctx: &egui::Context) -> Self {
        ...
        let mut skin = Skin::load(&settings.current_skin).unwrap_or_else(|_| Skin::load("default").unwrap());
        ...
        let song_button_selected_texture = load_egui_tex(skin.song_button_selected.clone(), "song_btn_sel");
        let difficulty_button_texture = load_egui_tex(skin.difficulty_button.clone(), "diff_btn");
        let difficulty_button_selected_texture = load_egui_tex(skin.difficulty_button_selected.clone(), "diff_btn_sel");
        ...
        let background_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            ...
            vertex: wgpu::VertexState { module: &bg_shader, entry_point: Some("vs_main"), buffers: &[], compilation_options: Default::default() },
            fragment: Some(wgpu::FragmentState { module: &bg_shader, entry_point: Some("fs_main"), targets: &[Some(wgpu::ColorTargetState { format: config.format, blend: Some(wgpu::BlendState::ALPHA_BLENDING), write_mask: wgpu::ColorWrites::ALL })], compilation_options: Default::default() }),
        ...
        let quad_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            ...
            layout: Some(&device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor { label: Some("Quad Layout"), bind_group_layouts: &[], push_constant_ranges: &[] })),
            ...
            fragment: Some(wgpu::FragmentState { module: &quad_shader, entry_point: Some("fs_main"), targets: &[Some(wgpu::ColorTargetState { format: config.format, blend: Some(wgpu::BlendState::ALPHA_BLENDING), write_mask: wgpu::ColorWrites::ALL })], compilation_options: Default::default() }),
        ...
        let def_col = [(receptor_color[0]*255.) as u8, (receptor_color[1]*255.) as u8, (receptor_color[2]*255.) as u8, (receptor_color[3]*255.) as u8];

        for col in 0..NUM_COLUMNS {
            let path = skin.get_receptor_image(NUM_COLUMNS, col);
            let tex = path.as_ref().and_then(|p| load_texture_from_path(device, &ctx.queue, p).map(|(t,_,_)| t))
                .unwrap_or_else(|| create_default_texture(device, &ctx.queue, def_col, "Def Receptor"));
            let view = tex.create_view(&Default::default());
            receptor_bind_groups.push(device.create_bind_group(&wgpu::BindGroupDescriptor {
                label: Some("Receptor BG"), layout: &bind_group_layout,
            ...
            let path_p = skin.get_receptor_pressed_image(NUM_COLUMNS, col).or(path.clone());
            let tex_p = path_p.as_ref().and_then(|p| load_texture_from_path(device, &ctx.queue, p).map(|(t,_,_)| t))
                .unwrap_or_else(|| create_default_texture(device, &ctx.queue, def_col, "Def Pressed"));
            let view_p = tex_p.create_view(&Default::default());
            receptor_pressed_bind_groups.push(device.create_bind_group(&wgpu::BindGroupDescriptor {
                label: Some("Pressed BG"), layout: &bind_group_layout,
                entries: &[wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(&view_p) }, wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::Sampler(&sampler) }]
            }));

            let path_n = skin.get_note_image(NUM_COLUMNS, col);
            let tex_n = path_n.as_ref().and_then(|p| load_texture_from_path(device, &ctx.queue, p).map(|(t,_,_)| t))
                .unwrap_or_else(|| create_default_texture(device, &ctx.queue, [(skin.colors.note_color[0]*255.) as u8, 255, 255, 255], "Def Note"));
            let view_n = tex_n.create_view(&Default::default());
            note_bind_groups.push(device.create_bind_group(&wgpu::BindGroupDescriptor {
                label: Some("Note BG"), layout: &bind_group_layout,
                entries: &[wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(&view_n) }, wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::Sampler(&sampler) }]
            }));
        }

        let font_path = skin.get_font_path().unwrap_or(PathBuf::from("assets/font.ttf"));
        let text_brush = load_text_brush(device, config.width, config.height, config.format, Some(&font_path));
        ...
        let colors = crate::models::stats::JudgementColors {
            marv: skin.colors.marv, perfect: skin.colors.perfect, great: skin.colors.great,
            good: skin.colors.good, bad: skin.colors.bad, miss: skin.colors.miss, ghost_tap: skin.colors.ghost_tap
        };

        let mut res = Self {
            render_pipeline, background_pipeline, quad_pipeline,
            instance_buffer, receptor_buffer, quad_buffer,
            note_bind_groups, receptor_bind_groups, receptor_pressed_bind_groups,
            background_bind_group: None, background_sampler: bg_sampler,
            current_background_path: None,
            
            song_button_texture, song_button_selected_texture,
            difficulty_button_texture, difficulty_button_selected_texture,
            
            text_brush, pixel_system, skin, settings,
    ...
    pub fn update_component_positions(&mut self, screen_width: f32, screen_height: f32) {
        ...
        let playfield_center_x = if let Some(pos) = config.playfield_pos { pos.x } else { screen_width / 2.0 };
        let playfield_offset_y = if let Some(pos) = config.playfield_pos { pos.y } else { 0.0 };
        ...
        let score_pos = config.score_pos.unwrap_or(UIElementPos { x: default_score_x, y: default_score_y });
        self.score_display.set_position(score_pos.x, score_pos.y);
        self.score_display.set_size(config.score_text_size);

        let combo_pos = config.combo_pos.unwrap_or(UIElementPos { x: playfield_center_x, y: default_combo_y });
        self.combo_display.set_position(combo_pos.x, combo_pos.y);
        self.combo_display.set_size(config.combo_text_size);

        let acc_pos = config.accuracy_pos.unwrap_or(UIElementPos { x: default_acc_x, y: screen_height * 0.1 });
        self.accuracy_panel.set_position(acc_pos.x, acc_pos.y);
        self.accuracy_panel.set_size(config.accuracy_text_size);

        let judge_pos = config.judgement_pos.unwrap_or(UIElementPos { x: default_acc_x, y: screen_height * 0.15 });
        ...
        let hitbar_pos = config.hit_bar_pos.unwrap_or(UIElementPos { x: playfield_center_x - hitbar_width / 2.0, y: combo_pos.y + 60.0 });
        self.hit_bar.set_geometry(hitbar_pos.x, hitbar_pos.y, hitbar_width, config.hit_bar_height_px);

        let flash_pos = config.judgement_flash_pos.unwrap_or(UIElementPos { x: playfield_center_x, y: combo_pos.y + 30.0 });
