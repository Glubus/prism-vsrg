...
impl Renderer {
    ...
    pub fn resize(&mut self, new_size: winit::dpi::PhysicalSize<u32>) {
        ...
        self.resources.pixel_system.update_size(new_size.width, new_size.height, None);
        self.resources.text_brush.resize_view(new_size.width as f32, new_size.height as f32, &self.ctx.queue);
        self.resources.update_component_positions(self.ctx.config.width as f32, self.ctx.config.height as f32);
    }
        ...
        if let RenderState::Menu(ref menu) = new_state {
            if let Some((set, _)) = menu.get_selected_beatmapset() {
                if let Some(img_path) = &set.image_path {
                    self.resources.load_background(&self.ctx.device, &self.ctx.queue, img_path);
                }
            }
        }
    ...
    pub fn render(&mut self, window: &Window) -> Result<Vec<GameAction>, wgpu::SurfaceError> {
        ...
        // 1. Game Layer
        draw_game(&self.ctx, &mut self.resources, &mut encoder, &view, &self.current_state, self.current_fps);
        ...
        match &self.current_state {
            RenderState::Menu(menu_state) => {
                ...
                if menu_state.show_settings {
                    ...
                    let previous_skin = settings.current_skin.clone();
                    let mut open = true;
                    
                    egui::Window::new("Settings").open(&mut open).show(&ctx_egui, |ui| {
                        ui.heading("Skin");
                            ...
                            for entry in entries.flatten() {
                                if entry.path().is_dir() {
                                    if let Some(name) = entry.file_name().to_str() {
                                        if name != "default" { skins.push(name.to_string()); }
                                    }
                                }
                            }
                        }
                        egui::ComboBox::from_label("Skin").selected_text(&settings.current_skin).show_ui(ui, |ui| {
                            for skin_name in skins {
                                ui.selectable_value(&mut settings.current_skin, skin_name.clone(), skin_name);
                            }
                        });
                        if ui.button("Save").clicked() { settings.save(); actions_to_send.push(GameAction::ToggleSettings); }
                    });
                    
                    if settings.current_skin != previous_skin {
                        settings.save();
                        self.resources = RenderResources::new(&self.ctx, &ctx_egui);
                        self.resources.update_component_positions(self.ctx.config.width as f32, self.ctx.config.height as f32);
                    }
                    if !open { actions_to_send.push(GameAction::ToggleSettings); }
                }

                let colors = &self.resources.skin.colors;
                let to_egui = |c: [f32; 4]| egui::Color32::from_rgba_unmultiplied((c[0]*255.) as u8, (c[1]*255.) as u8, (c[2]*255.) as u8, (c[3]*255.) as u8);
                let dummy_win = crate::models::engine::hit_window::HitWindow::new();
                
                let (action_opt, _) = self.song_select_screen.render(
                    &ctx_egui, menu_state, &view, 
                    self.ctx.config.width as f32, self.ctx.config.height as f32,
                    &dummy_win, self.resources.settings.hit_window_mode, self.resources.settings.hit_window_value,
                    self.resources.song_button_texture.as_ref().map(|t| t.id()),
                    self.resources.song_button_selected_texture.as_ref().map(|t| t.id()),
                    self.resources.difficulty_button_texture.as_ref().map(|t| t.id()),
                    self.resources.difficulty_button_selected_texture.as_ref().map(|t| t.id()),
                    to_egui(colors.selected_color), to_egui(colors.difficulty_selected_color)
                );

                if let Some(a) = action_opt {
                    match a {
                        UIAction::SetSelection(i) => actions_to_send.push(GameAction::SetSelection(i)),
                        UIAction::SetDifficulty(i) => actions_to_send.push(GameAction::SetDifficulty(i)),
                        UIAction::Select => actions_to_send.push(GameAction::Confirm),
                        UIAction::Back => actions_to_send.push(GameAction::Back),
                        UIAction::ToggleSettings => actions_to_send.push(GameAction::ToggleSettings),
                        _ => {}
                    }
                }
            }
            ...
            RenderState::Editor(snapshot) => {
                if let Some((target, mode, dx, dy)) = snapshot.modification {
                    ...
                    match (target, mode) {
                        // Redimensionnement
                        (EditorTarget::Notes, EditMode::Resize) => { config.note_width_px += dx*speed; config.note_height_px -= dy*speed; }
                        (EditorTarget::Receptors, EditMode::Resize) => { config.receptor_width_px += dx*speed; config.receptor_height_px -= dy*speed; }
                        (EditorTarget::Combo, EditMode::Resize) => config.combo_text_size -= dy * speed,
                        (EditorTarget::Score, EditMode::Resize) => config.score_text_size -= dy * speed,
                        (EditorTarget::Accuracy, EditMode::Resize) => config.accuracy_text_size -= dy * speed,
                        (EditorTarget::Judgement, EditMode::Resize) => config.judgement_text_size -= dy * speed,
                        (EditorTarget::HitBar, EditMode::Resize) => config.hit_bar_height_px -= dy * speed,
                        
                        // Déplacement (Valable pour TOUS les targets)
                        (t, EditMode::Move) => {
                            let pos_opt = match t {
                                EditorTarget::Notes | EditorTarget::Lanes | EditorTarget::Receptors => &mut config.playfield_pos,
                    ...
                    .show(&ctx_egui, |ui| {
                        ...
                        if let Some(target) = snapshot.target {
                            ...
                            let text = match (target, snapshot.mode) {
                                ...
                                (t, EditMode::Move) => {
                                    let pos = match t {
                                        EditorTarget::Notes | EditorTarget::Lanes | EditorTarget::Receptors => config.playfield_pos,
                                ...
                                (EditorTarget::Notes, EditMode::Resize) => format!("Size: W {:.0} H {:.0}", config.note_width_px, config.note_height_px),
                                (EditorTarget::Receptors, EditMode::Resize) => format!("Size: W {:.0} H {:.0}", config.receptor_width_px, config.receptor_height_px),
                                (EditorTarget::Combo, EditMode::Resize) => format!("Size: {:.0}", config.combo_text_size),
                                (EditorTarget::Score, EditMode::Resize) => format!("Size: {:.0}", config.score_text_size),
                                (EditorTarget::Accuracy, EditMode::Resize) => format!("Size: {:.0}", config.accuracy_text_size),
                                (EditorTarget::Judgement, EditMode::Resize) => format!("Size: {:.0}", config.judgement_text_size),
                                (EditorTarget::HitBar, EditMode::Resize) => format!("Height: {:.0}", config.hit_bar_height_px),
                                _ => "Mode not supported".to_string(),
                            };
                            ui.label(egui::RichText::new(text).color(egui::Color32::YELLOW).size(20.0));
                        }
