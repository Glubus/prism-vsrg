...
fn render_hit_histogram(
    ...
    let bottom_y = rect.bottom() - 20.0; // Marge pour le texte en bas
    ...
    // Dessin des barres
    let bar_width = (width / bucket_count as f32) * 0.8; // 0.8 pour laisser un petit espace entre les barres
    ...
    for (i, &count) in buckets.iter().enumerate() {
        ...
        let x_normalized = (i as f32 + 0.5) / bucket_count as f32; // +0.5 pour centrer la barre
        let center_bar_x = rect.left() + x_normalized * width;

        let bar_height_normalized = count as f32 / max_bucket_val as f32;
        let bar_height = bar_height_normalized * graph_height;
        ...
        // Couleur basée sur la distance au centre (0ms)
        // On calcule le temps correspondant au milieu du bucket
...
fn render_timeline_graph(
    ...
    let width = rect.width() - 40.0; // On garde de la place à droite pour les labels Y
    ...
    // Fonction helper pour dessiner ligne + texte
    let draw_guide = |ms: f64, color: Color32, label: &str| {
        let y_offset = ms as f32 * scale_y;
        let stroke = Stroke::new(1.0, color.linear_multiply(0.15));

        // Ligne Haut (Tard / Late)
    ...
    // Tu peux décommenter les suivants si tu veux plus de lignes, mais ça charge le graphe
    // draw_guide(hit_window.good_ms, Color32::from_rgb(0, 0, 128), "Good");
...
// Utilitaire pour récupérer la couleur selon le timing
fn get_color_for_timing(timing: f64, hit_window: &HitWindow) -> Color32 {
    ...
    if abs_timing <= hit_window.marv_ms {
        ...
        Color32::from_rgb(0, 0, 128) // Dark Blue
    } else if abs_timing <= hit_window.bad_ms {
        Color32::from_rgb(255, 105, 180) // Pink
    } else {
        Color32::RED
    }
}
