    ...
    pub fn render(
        ...
        if ui.is_rect_visible(rect) {
            ...
            if let Some(tex_id) = texture_id {
                ...
                let fill_color = if is_selected {
                    Color32::from_rgba_unmultiplied(selected_color.r(), selected_color.g(), selected_color.b(), 50)
                } else {
                    Color32::from_rgba_unmultiplied(0, 0, 0, 250)
                };
                painter.rect_filled(rect, 0.0, fill_color);

                let stroke_color = if is_selected { selected_color } else { Color32::BLACK };
        ...
        content_ui.vertical(|ui| {
            if let Some(title) = &beatmapset.title {
                ui.add(Label::new(RichText::new(title).size(24.0).strong().color(Color32::WHITE)).selectable(false));
            }
            ...
            ui.add(Label::new(RichText::new(&artist_creator).size(16.0).color(Color32::LIGHT_GRAY)).selectable(false));
        });
