...
impl std::fmt::Display for EditTarget {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            EditTarget::Judgement => write!(f, "JUDGEMENT"),
            EditTarget::Counter => write!(f, "COUNTER"),
...
impl EditorStateController {
    ...
    fn apply_change(&self, ctx: &mut StateContext, dx: f32, dy: f32) {
        ...
        ctx.with_renderer(|renderer| {
            ...
            let speed = if renderer.resources.settings.is_open {
                0.0
            } else {
                2.0
            ...
            match (target, self.current_mode) {
                ...
                (_, EditMode::Move) => {
                    ...
                    let p =
                        pos_opt.get_or_insert(crate::models::skin::UIElementPos { x: 0., y: 0. });
                    p.x += dx * speed;
                    p.y += dy * speed;
                }
                ...
                (EditTarget::Judgement, EditMode::Resize) => {
    ...
    fn handle_input(
        ...
        } = event
        {
            match key_code {
                ...
                KeyCode::KeyS => {
                    ctx.with_renderer(|renderer| {
                        if let Err(e) = renderer.resources.skin.save_user_config() {
            ...
            if let Some(target) = target_copy {
                ...
                let values_str = match target {
                    ...
                    EditTarget::HitBar => {
                        let p = config
                            .hit_bar_pos
                            .unwrap_or(crate::models::skin::UIElementPos { x: 0., y: 0. });
                        format!(
                            "X: {:.1} Y: {:.1} | H: {:.1}",
                            p.x, p.y, config.hit_bar_height_px
                        )
                    }
                    EditTarget::Combo => {
                        let p = config
                        ...
                        format!(
                            "X: {:.1} Y: {:.1} | Size: {:.1}",
                            p.x, p.y, config.combo_text_size
                        )
                    }
                    EditTarget::Score => {
                        let p = config
                        ...
                        format!(
                            "X: {:.1} Y: {:.1} | Size: {:.1}",
                            p.x, p.y, config.score_text_size
                        )
                    }
                    EditTarget::Accuracy => {
                        let p = config
                        ...
                        format!(
                            "X: {:.1} Y: {:.1} | Size: {:.1}",
                            p.x, p.y, config.accuracy_text_size
                        )
                    }
                    EditTarget::Judgement => {
                        ...
                        format!(
                            "X: {:.1} Y: {:.1} | Size: {:.1}",
                            p.x, p.y, config.judgement_text_size
                        )
                    }
                ...
                renderer.resources.editor_status_text = Some(
                    "SELECT: W(Note) X(Rec) C(Cmb) V(Scr) B(Acc) N(Judg) K(Bar) | S(Save)"
                        .to_string(),
                );
                renderer.resources.editor_values_text =
                    Some("Press key to select element".to_string());
            }
        });
        StateTransition::None
    }
